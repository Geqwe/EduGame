...
public class FourthGameManager : MonoBehaviour
{
    public static FourthGameManager instance;
    ...
    public void CheckPos() {
        ...
        if(btnGM.name == objectsSequence[position].gameObject.name) {
            position++;
            Right();
        }
        else {
    ...
    IEnumerator FadeOrderAndShape() {
        ...
        yield return new WaitForSeconds(2f);
        for (int i = 0; i < objectsSequence.Length; i++) {
            objectsSequence[i].CrossFadeAlpha(0, 0.5f, false);
            yield return new WaitForSeconds(0.5f);
            objectsSequence[i].CrossFadeAlpha(1, 0.5f, false);
            yield return new WaitForSeconds(0.5f);
            objectsSequence[i].CrossFadeAlpha(0, 0.5f, false);
            yield return new WaitForSeconds(0.5f);
            objectsSequence[i].CrossFadeAlpha(1, 0.5f, false);
        ...
        panel.SetActive(true);
        audio.PlayOneShot(order4);
        order.CrossFadeAlpha(0, 2, false);
        for (int i = 0; i < buttonsSequence.Length; i++) {
            buttonsSequence[i].interactable = true;
        }
        yield return new WaitForSeconds(2f);
    ...
    IEnumerator Fade() {
        ...
        SceneManager.LoadScene("FourthLevelSelect");
    }
